...
def getFig2(dfMerged):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['KB매수우위지수'], mode='lines+markers', name='KB매수우위지수'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['한국은행 기준금리 * 50'], mode='lines+markers', name='한국은행 기준금리 * 50'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['지가변동률 * 50'], mode='lines+markers', name='지가변동률 * 50'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['현금통화량 / 1000'], mode='lines+markers', name='현금통화량 / 1000'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['가계 통화량 / 10000'], mode='lines+markers', name='가계 통화량 / 10000'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['소비자물가지수(총지수)'], mode='lines+markers', name='소비자물가지수(총지수)'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['생산자물가지수'], mode='lines+markers', name='생산자물가지수'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['수입물가지수'], mode='lines+markers', name='수입물가지수'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['수출물가지수'], mode='lines+markers', name='수출물가지수'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['기대인플레이션 * 50'], mode='lines+markers', name='기대인플레이션 * 50'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['서울시 가계대출규모 합계 / 5000'], mode='lines+markers', name='서울시 가계대출규모 합계 / 5000'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['경제활동인구_취업자_합계 / 50'], mode='lines+markers', name='경제활동인구 취업자 합계 / 50'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['가상? 가처분소득 / 20'], mode='lines+markers', name='(소득-소비) / 20'))
    fig.update_layout(width=1200, height=800)
    return fig

def getFig3(dfMerged):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['KB매수우위지수'], mode='lines+markers', name='KB매수우위지수'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['한국은행 기준금리 * 50'], mode='lines+markers', name='한국은행 기준금리 * 50'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['지가변동률 * 50'], mode='lines+markers', name='지가변동률 * 50'))
...
def getFig5(dfMerged):
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['KB매수우위지수'], mode='lines+markers', name='KB매수우위지수'))
    fig.add_trace(go.Scatter(x=dfMerged['ym'], y=dfMerged['뉴스심리지수'], mode='lines+markers', name='뉴스심리지수'))
    return fig

def getLayoutComponent():
    dfMerged = getMergedDf()
